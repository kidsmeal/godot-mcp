...
| Tool | What it does |
|---|---|
| `procgen_tileset_build(config_path, out_path)` | Build a `.tres` TileSet from a declarative TOML config: atlas sources with animation-aware sheet scanning (frame regions are reserved, never tiled), terrain sets + peering bits via a `blob47` / `blob16_sides` / `blob16_corners` / `explicit` strategy, physics + typed custom-data layers (physics/nav layers are only created when the config actually declares them). Validates the water-bottom law (≤1 terrain per terrain set; water-bearing animated tiles must be `mode="default"`). Never calls the built-in terrain solver (#76493 / #89844) — bits are assigned directly for the in-house matcher |
| `procgen_terrain_audit(tileset_path, terrain_set=-1)` | Audit a `.tres` TileSet's terrain-peering-bit coverage against the water-bottom law: expected signature-class set per terrain set (derived from the mode's valid peering bits, e.g. 47/16 classes — never hardcoded), covered / missing / duplicated signatures (duplicates = variants, allowed), broken ordering (`terrain != -1` but `terrain_set == -1`), >1-terrain-per-set law violations, unused tiles, and animation-desync linting (every animated terrain tile must share frames/duration + `mode=DEFAULT`, else error). Returns a markdown table plus a machine `coverage` dict the game repo's in-house matcher consumes |
| `procgen_atlas_grid(sheet_path, tile_size=8, scale=6, region="")` | Render a tilesheet upscaled with a per-tile coordinate grid overlay, so tile coordinates (plain tile, variants, edge-block origin) can be read off directly for procgen configs — no headless Godot involved (pure Pillow, unlike every other procgen_* tool). Checkerboard-composites transparent cells, upscales NEAREST for crisp pixel edges, draws per-tile gridlines (every 5th accented), and labels columns/rows along the top/left. `region` optionally crops a tile-coord window (`"25:32,3:8"` = cols 25-31, rows 3-7). Returns real MCP image content plus a short text summary |
