...
- [ ] **NEXT: Procgen P1 — `procgen_tileset_build`** (`.tres` TileSet from a declarative per-pack config; `blob47` peering-bit table; animation-aware sheet scan reserving frame regions; one-terrain-per-terrain-set validation per the water-bottom law; reload sanity check). Deletes the throwaway `procgen_ping` (+ its roster/test entries) as the first real tool lands. **Carry-in from the P0 review (already folded into the plan's Phase 1 section):** before copying the harness, harden the sentinel against collision (per-run nonce or length-frame the JSON — a real payload could contain the literal end-marker) and relax the "one line" docstring wording. First action: gantry:build procgen Phase 1. **Pre-check:** confirm the real Minifantasy ForgottenPlains sheet path exists (`C:\Users\atk67\Documents\minifantasyassets\Minifantasy_ForgottenPlains_*`) to build against.
