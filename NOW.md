...
- [ ] **NEXT: Procgen P3 — S1 matcher spike (GATE)**: throwaway headless script builds a ~20×20 toy landmass mask (bay + interior pond), resolves every ground cell through the matcher algorithm (8-neighbor same-layer signature → exact-match against the P2 `coverage` table → seeded variant pick → full-interior fallback on miss), asserts interior/straight-edge/inner-corner/outer-corner picks vs hand-computed expectations, times a 96×96 resolve, renders one eyeball pass. Write the signature-computation helper as if it were the game matcher's core (game implements `terrain_matcher.gd` as a port). Record the verdict in `procgen-tools.md` under an "S1 verdict" heading. **Unblocks the game plan's phase 3.** First action: gantry:build procgen Phase 3.
