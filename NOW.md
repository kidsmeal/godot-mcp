...
- [x] **`interior` param SHIPPED** `33e3042` — interior ground cells get the all-4-corners signature (the 16th class); MULTIPLE = variants; clean 16/16, waffle gone. Restricted to `minifantasy_edges` (codex-caught scope hole fixed). Real plains (grass) + desert (SAND, from the sand-water block @ ~(20,17)) build to 16/16 + render as proper islands (orchestrator eyeballed). Reviewed codex PASS.
- [x] **Per-variant weighting SHIPPED** `b3e0207` — `[col,row,weight]` interior entries → `weight` TYPE_FLOAT custom-data layer → audit `coverage` dict → deterministic WEIGHTED matcher pick (scale-invariant; equal weights == prior uniform). `weight` is a reserved float layer. Real plains weighted render is mostly plain grass + sparse tufts (0.917/0.036/0.047 for 0.9/0.05/0.05), vs the even-scatter control. Reviewed codex PASS (after a weight-layer contract fix). 434 passed / 7 skipped. **Biome-ground authoring is now complete: edge origin + weighted interior variants → a natural 16/16 ground tileset.**
- [ ] **OPEN QUESTION (user, "select the plain tile"):** clarifying whether they want (a) a `procgen_atlas_grid` **inspector** tool (render any sheet with a coordinate grid so they pick tile coords themselves — the parked helper), (b) a **`base`/`plain` config field** (designate the dominant fill tile, auto-gets remaining weight), or (c) both. Decides the next build.
- [ ] **REMAINING for a full biome tileset:** the 2-frame water animation via `[[animation]]` (edge geometry + interior + weighting all done; frame handling — incl. the non-adjacent-frame repack — is the last scaffolding).
- [ ] **PARKED (game-gated):** plan phases 4–6 (`procgen_worldgen_preview`/`island_preview`/`chunk_lint`/`gen_smoke`) need the game repo `squareds/project1` dump hooks (game plan phases 2–3).
