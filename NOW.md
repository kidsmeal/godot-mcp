...
- [ ] **NEXT — decision point (procgen is at a natural pause):** plan phases 4–6 (`procgen_worldgen_preview` / `procgen_island_preview` / `procgen_chunk_lint` / `procgen_gen_smoke`) are **gated on the game repo `squareds/project1` providing dump hooks** (game plan phases 2–3), so they can't build here yet. The buildable mcp-side options are: (a) add the `blob15_3x5`/`minifantasy_edges` strategy so real Minifantasy biome tilesets build through T1 without hand-`explicit` configs (+ the frame-repack for non-adjacent animation frames); (b) the `procgen_atlas_grid` inspector helper (the grid-overlay used to read the sheets); (c) switch threads (Queue: Phase 7 runtime loop, Feature batch I, etc.). Carry: verify the 4 imported biome tilesets' water = `#4070d0` + 2 frames once their `.tres` exist (color already verified; animation pending build).
