...
Phases 1–6.6 shipped: containment, validator-harness leak fix, engine-grounding correctness, profile robustness, ruff/mypy/CI, Feature F grounding, bridge hardening + auth, the bucket-C audit hardening batch (6.5), and the tool-surface settle (6.6). **301 tests green; `main` is 4 commits AHEAD of `origin/main` (`1bc6b96`..`42c0a37`, the 6.6 stack) — UNPUSHED, awaiting human review + push.** Plan: `docs/design/feature-batch-fgh.plan.md`. Loop per phase: implementer → phase-reviewer → fix/re-review → commit + push (human-gated at each commit).
...
- [x] **Phase 6.6 — tool-surface settle** SHIPPED 2026-07-03 (all 3 phases, gated build, committed locally — **UNPUSHED, pending human review + push**). Plan appended to `docs/design/feature-batch-fgh.plan.md`; each phase ran implementer → phase-reviewer (opus) → commit.
    - [x] P1 — rename 5 editor-bridge tools `godot_*`→`editor_*` across server.py + README + templates + ci_smoke roster + tests. `56f3a4d`. Reviewer PASS-WITH-NOTES (scope-only note: cursor/plan rode alongside; split into its own commit `1bc6b96`). 280 passed / 7 skipped.
    - [x] P2 — FastMCP `instructions=` block (res:// rule, "Refused" shape, `# lint: ignore` syntax, ground→edit→confirm loop) + docstring slim (only redundant `res://` sentences, 6770→6517 chars) + 6 delivery tests. `aa9c09e`. Reviewer PASS (block verified accurate vs code). 286 passed / 7 skipped.
    - [x] P3 — `godot_class`/`godot_member` first-sentence doc descriptions by default + `full_docs=True` param; C22 (P4) left byte-identical; 15 new tests incl. C22-tail regression. `42c0a37`. Reviewer PASS (C22 byte-identical confirmed, real-input checked). 301 passed / 7 skipped.
- [ ] **NEXT: human reviews the 4 unpushed commits (`1bc6b96`..`42c0a37`) + pushes.** Then Phase 6.6 is fully done and the procgen suite (Queue item 1) becomes eligible. Before procgen build starts, settle its tool-family prefix (`procgen_*` vs `worldgen_*`) — 6.6 established the family-prefix convention but did not name the procgen family; that micro-decision belongs at procgen Phase 0.

**Drift note (2026-07-03):** NOW.md sat untouched from 2026-06-10 while a full procgen tool-suite design+plan (`docs/design/procgen-tools.md` + `.plan.md`, commit `2830a37`) was written and committed. That thread is real and queued (Queue item 1 below) — user confirmed it "needs to be implemented at one point" but Phase 6.6 stays the one active thread first. Coupling: procgen adds 6 new tools with bare names; its family prefix should be settled under 6.6's naming scheme, not accreted separately.
...
> Shipped/active items pruned 2026-07-03: Phase 6 and Phase 6.5 shipped (see SHIPPED.md / active-thread changelog); Phase 6.6 is now the one active thread (top of file), no longer queued.

1. **Procgen tool suite** (fully designed + planned — `docs/design/procgen-tools.md` v2 + `docs/design/procgen-tools.plan.md` v2, both 2026-07-03; research record `docs/research/godot_tilemap_procgen_research.md`, 23 sources / 25 claims 3-vote verified). Headless tileset build + biome island generation, 6 tools: T1 `tileset_build` (declarative `.tres` TileSet from a per-pack config), T2 `terrain_audit` (signature-coverage QA + matcher dataset), T3 `worldgen_preview`, T4 `island_preview`, T5 `chunk_lint`, T6 `gen_smoke`. Design laws locked: **water-bottom law** (every layer autotiles itself-vs-empty only, one terrain per terrain set), **in-house matcher is the decided autotiling backend** (BetterTerrain fallback only), previews composed in Python/PIL (headless engine render is a credible dead end). Plan phases 0-3 are self-contained in THIS repo (Phase 0 recon+harness → T1 → T2 → S1 matcher-spike GATE); plan phases 4-6 (`worldgen_preview`/`island_preview`/`chunk_lint`/`gen_smoke`) are gated on the game repo `C:\Users\atk67\Documents\squareds\project1` providing dump hooks. Done = phases 0-3 shipped (T1/T2 green + S1 verdict recorded in the design doc), which unblocks the game plan's phase 3. **Naming — DECIDED 2026-07-03:** the family is **`procgen_*`** (`procgen_tileset_build`, `procgen_terrain_audit`, `procgen_worldgen_preview`, `procgen_island_preview`, `procgen_chunk_lint`, `procgen_gen_smoke`); recorded in both procgen docs + stance rule 4. First action: gantry:build Phase 0 (recon + harness) — add the module + a throwaway `procgen_ping`, prove a headless JSON round-trip, add each real tool to `ci_smoke` EXPECTED_TOOLS as it lands.
