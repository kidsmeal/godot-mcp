...
- [x] Procgen P1 — `procgen_tileset_build`: declarative `.tres` TileSet builder; `blob47`/`blob16` peering-bit tables from first principles (256→47 via corner-implies-both-sides), animation-aware scan, load-bearing op-order, per-run nonce sentinel (hardened, shared harness), `procgen_ping` retired. Live-verified vs 4.6.2 (synthetic fixture + real ForgottenPlains one-off, 567 tiles). `d904466`. Reviewed **PASS** (opus fallback — codex was blocked by a `~/.codex/config.toml` `service_tier="priority"` incompatibility, see below). 330 passed / 7 skipped.
- [ ] **NEXT: Procgen P2 — `procgen_terrain_audit`** (`terrain_audit(tileset_path, terrain_set=-1)`: load a built `.tres`, dump per-tile terrain + animation, report per terrain the expected vs covered/missing/duplicated signatures, broken-ordering tiles, >1-terrain law violations, unused tiles, and the animation-sync lint; emit a stable machine `coverage` dict the game-repo matcher consumes). **Carry-ins from the P1 review (folded into the plan's Phase 2 section):** collapse the duplicated frame_offset guard to one source, and make the physics/nav layer creation conditional (don't bake an empty nav layer into every tileset). First action: gantry:build procgen Phase 2.
- [ ] **Blocked-tool note:** codex is the committed standing reviewer (`.gantry/models.json`) but can't run until `~/.codex/config.toml`'s `service_tier` is fixed (currently `"priority"`; codex-cli 0.130.0-alpha.5 accepts only `fast`/`flex`, or delete the line). Until then reviews fall back to native opus.
