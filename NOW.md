...
- [~] **Phase 6.6 — tool-surface settle** — ACTIVE (confirmed 2026-07-03 over the procgen thread; see drift note below). Scope: (a) apply rename map + README + agent templates + ci_smoke roster in one diff; (b) MCP server `instructions` block + docstring slim pass; (c) response caps on fat tools (C22 godot_class budget [partly done P4], doc-desc first-sentence default + full_docs param). phase-planner dispatched 2026-07-03 to append a phased 6.6 plan to `docs/design/feature-batch-fgh.plan.md` (P1 rename / P2 instructions+slim / P3 caps). Next: review the plan, then gated build per phase (implementer → phase-reviewer → fix), leave uncommitted for the human commit gate.

**Drift note (2026-07-03):** NOW.md sat untouched from 2026-06-10 while a full procgen tool-suite design+plan (`docs/design/procgen-tools.md` + `.plan.md`, commit `2830a37`) was written and committed. That thread is real and queued (Queue item 4 below) — user confirmed it "needs to be implemented at one point" but Phase 6.6 stays the one active thread first. Coupling: procgen adds 6 new tools with bare names; its family prefix should be settled under 6.6's naming scheme, not accreted separately.
