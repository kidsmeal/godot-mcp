...
**Exit criteria:** A biome-set config validates in Python and produces a loadable `.tres` whose transition-panel tiles carry the expected corner-Wang bits; the golden build test proves transparent-outside == opaque-outside for the builder (the §3.1 "does the transparent-center block need a builder change?" question is answered NO with a passing test, or — if the test surprises us — a specific, isolated builder fix is scoped as its own micro-phase before continuing).
**Blockers:** none.
