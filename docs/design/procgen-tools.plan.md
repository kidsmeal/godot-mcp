...
- Add each tool to the mcp README's tool table. **Agent-grant scope (decided 2026-07-03):** procgen tools are registered on the MCP server + listed in the README + pinned in the `ci_smoke` roster, but they are **deliberately NOT added to the `godot-editor` agent grant** (`agent/templates/godot-editor.md.tmpl`) — that subagent drives the live-editor/grounded-edit loop, and procgen is a separate headless build/generation concern. The main assistant can call procgen tools directly (not bound by any subagent allowlist), and cross-project use works by onboarding the target project (`setup.ps1 -Project`). A **dedicated `godot-procgen` agent grant is deferred until the suite (T1–T6) is complete**, so its allowlist can carry the whole family at once rather than being amended every phase.
