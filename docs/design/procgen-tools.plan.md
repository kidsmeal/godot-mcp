...
**Carry-in from the Phase 1 review (2026-07-03) — clean these up in the shared harness before the compose pattern is copied a third time:**
- **Duplicated frame_offset contiguity guard.** `procgen.py` checks the `[1,0]`/`[0,1]` contiguity rule in BOTH `validate_config` and `compose_build_script`, so the compose-side check is now unreachable dead code that can silently drift from the validation-side check. Collapse to one source of truth (validate-only) as part of Phase 2's refactor so Phases 2–6 don't each copy a diverging pair.
- **Unconditional physics + nav layers.** `tileset_build` always adds one physics layer and one navigation layer even when the config declares no collision and never uses navigation, baking an empty nav layer into every generated TileSet. Make layer creation conditional on the config actually declaring them, so the family default isn't "every tileset carries an unused nav layer."
...
- Add each tool to the mcp README's tool table. **Agent-grant scope (decided 2026-07-03):** procgen tools are registered on the MCP server + listed in the README + pinned in the `ci_smoke` roster, but they are **deliberately NOT added to the `godot-editor` agent grant** (`agent/templates/godot-editor.md.tmpl`) — that subagent drives the live-editor/grounded-edit loop, and procgen is a separate headless build/generation concern. The main assistant can call procgen tools directly (not bound by any subagent allowlist), and cross-project use works by onboarding the target project (`setup.ps1 -Project`). A **dedicated `godot-procgen` agent grant is deferred until the suite (T1–T6) is complete**, so its allowlist can carry the whole family at once rather than being amended every phase. When built, that agent should expose **customizable parameters** the way the existing templates take project-specific values at onboard (`setup.ps1 -Project`): candidates — asset-pack root (e.g. the Minifantasy pack dir), default tile size / atlas conventions, default output dirs for generated `.tres`/previews, default blob strategy, and the target project. Design the parameter set when the suite lands; do not build the agent before then.
